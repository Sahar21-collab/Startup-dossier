...
5. No marketing language. Do not use words like "revolutionary", "game-changing", "disruptive", "innovative", "cutting-edge", "seamless", "world-class", "leading", "empower", "streamline", "unlock", "transform", "end-to-end", "next generation". Never copy a company's own slogan. Write specific, concrete, plain sentences, the way a journalist explains a business to a reader who knows nothing about the field. Prefer facts, mechanisms and numbers over adjectives.
...
const JARGON = /\b(revolutionar\w*|game[- ]?chang\w*|disrupt\w*|innovat\w*|cutting[- ]edge|state[- ]of[- ]the[- ]art|seamless\w*|world[- ]class|streamlin\w*|empower\w*|unlock\w*|leverag\w*|end[- ]to[- ]end|next[- ]generation|best[- ]in[- ]class|one[- ]stop|holistic|synerg\w*)\b/gi;
      ...
      if (words.length && !retried && Date.now() + CALL_TIMEOUT_MS < deadline) {
        ...
        body.contents.push(
          { role: "model", parts: [{ text: JSON.stringify({ problemSolved: result.problemSolved, valueAdded: result.valueAdded }) }] },
          {
            role: "user",
            parts: [{
              text: `Your answer used these banned marketing words: ${words.join(", ")}. Write the whole answer again, keeping every fact identical and the same level of detail, but replacing those words with plain descriptions of what actually happens.`,
            }]
          }
        );
        continue;
